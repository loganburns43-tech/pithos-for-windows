...
def _startup_error_dialog(msg, details=None):
        try:
                if os.name == 'nt':
                        import ctypes
                        flags = 0x00000010 | 0x00000000  # MB_SYSTEMMODAL | MB_OK
                        text = msg if details is None else u"%s\n\n%s" % (msg, details)
                        ctypes.windll.user32.MessageBoxW(0, text, u"Pithos", flags)
                        return
        except Exception:
                pass
...
try:
        ...
        _startup_error_dialog("Pithos failed to start", traceback.format_exc())
        sys.exit(1)
        ...
        def fatal_dialog(msg, submsg=None):
                if GTK_AVAILABLE:
                        try:
                                ...
                                pass
                _startup_error_dialog(msg, submsg)
